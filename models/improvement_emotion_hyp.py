...
from .improvement_hyp_crossvit import ProductCrossDualAttention
...
class pyramid_trans_expr(nn.Module):
    def __init__(self, img_size=224, num_classes=7, type="large"):
        ...
        self.dual_attn_fuse = ProductCrossDualAttention(
            dim=512,
            num_heads=8,
            attn_drop=0.0,
            proj_drop=0.1,
    ...
    def forward(self, x):
        ...
        y_hat = self.dual_attn_fuse(x_ir, x_face)
        y_hat = self.se_block(y_hat)
        y_hat = self.fusion_norm(y_hat)
        y_feat = y_hat
        out = self.head(y_hat)
